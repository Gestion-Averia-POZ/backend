...
const publicCreateRequestSchema = z.object({
  applicantName: z.string()
    .min(2, 'El nombre debe tener al menos 2 caracteres')
    .max(150, 'El nombre no puede tener más de 150 caracteres')
    .trim(),
...
export const validatePublicCreateRequest = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const validatedData = await publicCreateRequestSchema.parseAsync(req.body);
    req.body = validatedData;
    next();
  } catch (error) {
    if (error instanceof z.ZodError) {
      return res.status(400).json({
        success: false,
        message: 'Datos de solicitud pública inválidos',
        errors: error.errors.map(err => ({
          field: err.path.join('.'),
          message: err.message
        }))
